resource "azurerm_public_ip" "pip_vm1_pip1" {
  ...
  resource_group_name = azurerm_resource_group.rg_iut.name
  location            = azurerm_resource_group.rg_iut.location
...
resource "azurerm_network_interface" "nic_vm1_nic1" {
  ...
  ip_configuration {
    name                          = "primary"
    subnet_id                     = azurerm_subnet.snet_vnet1_iut1.id
    private_ip_address_allocation = "Dynamic"
    public_ip_address_id          = azurerm_public_ip.pip_vm1_pip1.id
  }
}

resource "azurerm_linux_virtual_machine" "vm_iut_vm1" {
  name                            = "vm-iut-vm1-frc"
  resource_group_name             = azurerm_resource_group.rg_iut.name
  location                        = azurerm_resource_group.rg_iut.location
  size                            = "Standard_B1s"
